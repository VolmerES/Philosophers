...
/*   Updated: 2024/10/24 17:07:07 by jdelorme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_error_mutex(t_code code, int status)
{
	...
	if (status == EINVAL)
		ft_error_exit(RED "Error: The value specified by mutex is invalid\n" RESET);
		...
		ft_error_exit(RED "Error: The current thread does not own the mutex\n" RESET);
	else
...
void	ft_error_thread(t_code code, int status)
{
	if (status == 0)
		...
		ft_error_exit(RED "Error: No thread with the ID thread could be found\n" RESET);
	else if (code == JOIN && status == EDEADLK)
		ft_error_exit(RED "Error: A deadlock was detected\n" RESET); 
	else if (code == CREATE && status == EPERM)
		ft_error_exit(RED "Error: No permission to set the scheduling policy"
